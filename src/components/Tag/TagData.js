// TagData.js
import image1 from '../../images/code.svg';
import image2 from '../../images/ganalytics.svg';
import image3 from '../../images/gtagmanager.svg';

const TagData = [
{
  id: 1,
  imageSrcCard: image1,
  imageSrcModal: image2,
  imageSrcModal2: image3,
  description: 'GTM tag was fired bitch!!!!!',
  // category: 'Analytics'
  successInfo: 'Wow you did it bitch!',
  additionalInfo: 'It is like this and like that and it is awesome bro!'
},

{
  id: 2,
  imageSrcCard: image3,
  imageSrcModal: image1,
  imageSrcModal2: image2,
  description: 'Muito preeeeeetoooooo',
  successInfo: 'tetinhaaaaaaaaaa',
  additionalInfo: 'aierhaiureuanb'
},

{
  id: 3,
  imageSrcCard: image2,
  imageSrcModal: image2,
  imageSrcModal2: image2,
  description: 'aiurenv',
  successInfo: 'bitch!',
  additionalInfo: 'mutherfucker safadao'
}

];

export default TagData;



// // TagData.js
// const tagData = {
//     imageSrc: '../../images/cat_no_bg.png', // Replace with the actual path to your image
//     description: 'GTM tag was fired bitch!!!!!',
//     category: 'Analytics'
//   };
  
//   export default tagData;
  